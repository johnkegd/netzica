                        ...
                        <div class="self-stretch justify-start items-start gap-2 inline-flex flex-wrap">
                            @for (primaryColor of primaryColors(); track primaryColor.name) {
                                <button
                                    ...
                                    [ngStyle]="{
                                        'background-color': primaryColor.name === 'noir' ? 'var(--text-color)' : '#500',
...
export class ThemeSwitcher {
  ...
  themeState = signal<ThemeState | null>(null);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.handleDarkModeTransition(state as ThemeState);
    });
  }
  ...
  primaryColors = computed(() => {
    const presetPalette = presets[(this.themeState()?.preset || 'Aura') as keyof typeof presets].primitive;
    const colors = [
      'emerald',
      'green',
      'lime',
    ...
    colors.forEach((color) => {
      palettes.push({
        name: color,
        palette: presetPalette?.[color as keyof typeof presetPalette] as any
      });
    });
